...
-- Returns aggregated stats, bypasses RLS, restricted to admin emails
...
begin
  ...
  select json_build_object(
    ...
    'popular_bikes', (select coalesce(json_agg(row_to_json(bk)), '[]'::json) from (
      select make, model, count(*) as rider_count from user_bikes group by make, model order by count(*) desc limit 10
    ) bk),
